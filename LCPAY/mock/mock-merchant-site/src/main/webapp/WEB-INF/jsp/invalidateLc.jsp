...
<head>
	<% 
		pageContext.setAttribute("title", "银信证交易测试-失效接口");
	%>
...
<body onload="">
    ...
    <!--内容-->
    <div class="container">
        <p class="raya-firstpay">失效接口测试页面(<span class="requireicon">*</span>标记为必填参数)</p>
        <div class="raya-ordertable">
		<form action="<c:url value="/lc/invalidate.lc" />" method="post" target="_blank" onsubmit="return validate()">
			<ul>
	            <li>
					<label>银信证ID：</label>
				...
				<li>
					<label>失效类型：</label>
					<select class="raya-select"  id="invalidateType" name="invalidateType" required >
						<option value="1" selected="selected">撤回失效</option>
						<option value="2" >退回失效</option>
						<option value="3" >退款(执行解付后)</option>
						<option value="4" >多次履约失效</option>
					</select><span class="requireicon">*</span>
				</li>
				<li>
					<label>履约ID：</label>
					<input class="raya-input"  id="lcAppointmentId" name="lcAppointmentId" value="${param.lcAppointmentId }" maxlength="200" />
				</li>
				<li>
					<label>退款订单号：</label>
