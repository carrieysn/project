...
public class HttpClient {
	...
	public int send(Map<String, String> data, String encoding) throws Exception {
		try {
			...
			LogUtil.writeLog("请求报文:[" + StringTool.printCard(sendData) + "]");
			this.requestServer(httpURLConnection, sendData,
					encoding);
			this.result = this.response(httpURLConnection, encoding);
			LogUtil.writeLog("同步返回报文:[" + StringTool.printCard(result) + "]");
		...
		try {
			...
			if (200 == status) {
				...
				if (null != resultString && !"".equals(resultString)) {
					// 将返回结果转换为map
					Map<String,String> tmpRspData  = DataUtil.convertResultStringToMap(resultString);
	...
	private String getRequestParamString(Map<String, String> requestParam, String coder) {
		...
		LogUtil.writeLog("请求报文(已做过URLEncode编码):[" + StringTool.printCard(reqstr) + "]");
		return reqstr;
	}
