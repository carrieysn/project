...
import com.cifpay.lc.bankadapter.unionpay.util.CertUtil;
 ...
 */
@Component
public class UnionPayTokenConsumeSMSDeal implements IBankDeal {

	private static Logger LOGGER = LoggerFactory.getLogger(UnionPayTokenConsumeSMSDeal.class);
	...
	public String getBankCode() {
		...
		return TradeConstant.TRADE_CONFIG.TRADE_UNION_PAY + TradeConstant.TRADE_CONFIG.TRADE_UNIONPAY_WU_SMS;
	}
	...
	public AbsBusinessResult bankDeal(AbsTradeParam tradeParam) throws Exception {
		...
		LOGGER.info("银联无跳转 -短信交易请求数据！ {}", StringTool.printMap(requestData));
		...
		LOGGER.info("银联无跳转 -短信交易请求返回数据！{} ", StringTool.printMap(responseData));
	...
	// 记录银联交易渠道流水信息
	public TrdUnionPayFlow insertUnionPayFlow(SmsCifParam param) {
		...
		payFlow.setTxnType(UnionPaySettingUtil.token_sms_txnType);
		payFlow.setTxnSubType(UnionPaySettingUtil.token_sms_txnSubType);
	...
	public Map<String, String> transToReqData(SmsCifParam param) throws Exception {
		return transToReqData4Level2(param);
	}

	public Map<String, String> transToReqData4Level2(SmsCifParam param) throws Exception {
		...
		inputParam.put("txnType", UnionPaySettingUtil.token_sms_txnType);
		inputParam.put("txnSubType", UnionPaySettingUtil.token_sms_txnSubType);
		...
		// 需要上送验证码，需要上送customerInfo域 modify by linql 2017.01.10
		...
		inputParam.put("encryptCertId",CertUtil.getEncryptCertId());   
