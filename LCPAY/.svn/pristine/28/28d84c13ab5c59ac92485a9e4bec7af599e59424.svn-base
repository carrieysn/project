 ...
 */
@Component
public class UnionPayTokenAuthUndoDeal implements IBankDeal {

	private static Logger LOGGER = LoggerFactory.getLogger(UnionPayTokenAuthUndoDeal.class);
	...
	public String getBankCode() {
		...
		return TradeConstant.TRADE_CONFIG.TRADE_UNION_PAY + TradeConstant.TRADE_CONFIG.TRADE_UNIONPAY_WU_AUTH_UNDO;
	}
	...
	public AbsBusinessResult bankDeal(AbsTradeParam tradeParam) throws Exception {
		...
		LOGGER.info("无跳转Token- 预授权撤销交易请求数据！ {}", StringTool.printMap(requestData));
		...
		LOGGER.info("无跳转Token- 预授权撤销交易请求返回数据！ {}", StringTool.printMap(responseData));
	...
	public TrdUnionPayFlow insertUnionPayFlow(ExpiryCifParam param) {
		...
		payFlow.setTxnType(UnionPaySettingUtil.token_auth_undo_txnType);
		payFlow.setTxnSubType(UnionPaySettingUtil.token_auth_undo_txnSubType);
	...
	public Map<String, String> transToReqData4Level2(ExpiryCifParam param) {
		...
		inputParam.put("txnType", UnionPaySettingUtil.token_auth_undo_txnType);
		inputParam.put("txnSubType", UnionPaySettingUtil.token_auth_undo_txnSubType);
		...
		inputParam.put("txnAmt", String.valueOf(param.getTxnAmt()));// 与原始预授权交易一致
