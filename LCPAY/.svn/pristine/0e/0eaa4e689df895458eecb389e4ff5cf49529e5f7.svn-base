 ...
 */
@Component
public class UnionPayTokenAuthFinishDeal implements IBankDeal {
	...
	public String getBankCode() {
		...
		return TradeConstant.TRADE_CONFIG.TRADE_UNION_PAY + TradeConstant.TRADE_CONFIG.TRADE_UNIONPAY_WU_AUTH_FINISH;
	}
	...
	public AbsBusinessResult bankDeal(AbsTradeParam tradeParam) throws Exception {
		...
		LOGGER.info("无跳转Token-预授权完成交易请求数据！ {}", StringTool.printMap(requestData));
		...
		LOGGER.info("无跳转Token-预授权完成交易请求返回数据！ {}", StringTool.printMap(responseData));
		...
		if (!responseData.isEmpty()) {
			if (EncryptDataUtil.validate(responseData, UnionPaySettingUtil.encoding_UTF8)) {
				...
				result.setQueryId(authFinishResult.getQueryId());// 消费交易后返回
																	// 交易查询流水号
																	// 供查询
	...
	public void updateUnionPayFlow(AuthHandleResult result, long flowId) {
		...
		updataFlow.setNewFlowId(result.getQueryId());// 预授权完成交易的交易流水号
	...
	public Map<String, String> transToReqData(PayCifParam param) {
		return transToReqDataLevel2(param);
	}

	public Map<String, String> transToReqDataLevel2(PayCifParam param) {
	...
	public Map<String, String> transToReqDataLevel1(PayCifParam param) {
