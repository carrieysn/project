...
<body onload="initURL()">
    ...
    <!--内容-->
    <div class="container">
        <p class="raya-firstpay">缓存接口测试页面(<span class="requireicon">*</span>标记为必填参数)</p>
        <div class="raya-ordertable">
		<form action="<c:url value="/lc/cache.lc" />" method="post" target="_blank" onsubmit="return validate()">
			<ul>
	            <li>
					<label>缓存key：</label>
					 <input class="raya-input" type="text" id="cacheKey" name="cacheKey"  value="LC_BANK_" maxlength="20" required /> <span class="requireicon">*</span>
				</li>
				
				<li>
					<label>businessParam：</label>
					<input class="raya-input" id="businessParam" name="businessParam" value="ICBC" maxlength="16" required /> <span class="requireicon">*</span>
				</li>
			</ul>
			<input class="raya-submit" value="确认" type="submit">
		</form>
		</div>
