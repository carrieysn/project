...
<head>
	<% 
	pageContext.setAttribute("title", "银信证交易测试-展期接口");
	%>
...
<body onload="">
    ...
    <!--内容-->
    <div class="container">
        <p class="raya-firstpay">展期接口测试页面(<span class="requireicon">*</span>标记为必填参数)</p>
        <div class="raya-ordertable">
		<form action="<c:url value="/defer.lc" />" method="post" target="_blank" onsubmit="return validate()">
			<ul>
	            <li>
					<label>银信证ID：</label>
			...
			<input class="raya-submit" value="展期提交" type="submit">
		</form>
		</div>
