...
import com.cifpay.lc.std.interceptor.LcAutoSendSMSInterceptor;
...
public class OpenLcNotifyMessageServiceImpl extends LcMessageServiceBase<LcFreezeParamBean>
    ...
    @Autowired
    private LcAutoSendSMSInterceptor lcAutoSendSMSInterceptor;
    ...
    protected BusinessOutput<MessageOutputBean> handleMessage(LcFreezeParamBean lcFreezeParamBean, CoreBusinessContext context) throws CoreBusinessException {
        ...
        List<LcOpen> lcOpenList = lcOpenDao.selectByLcId(lcFreezeParamBean.getLcId());
        if (lcOpenList != null && !lcOpenList.isEmpty()) {
            for (LcOpen lcOpen : lcOpenList) {
                if (lcOpen.getLcOpenStatus() != null && LcTranStatus.SUCCESS.getTranStatusStr().compareTo(lcOpen.getLcOpenStatus()) == 0) {
                    logger.warn("开证记录已经成功处理过，忽略请求: {}", lcFreezeParamBean);
                    // 消息处理成功
        ...
        if (LcTranStatus.SUCCESS.compareTo(lcFreezeParamBean.getLcTranStatus()) == 0) {
            ...
            lcAutoSendSMSInterceptor.afterProcessBusiness(this, lcFreezeParamBean, BusinessOutput.success(null), context);
        }
    ...
    private BusinessOutput notify(LcFreezeParamBean messageParams, Lc lc, PreLc preLc) {
        ...
        notifyBean.setMessage(messageParams.getFreezeDesc());
