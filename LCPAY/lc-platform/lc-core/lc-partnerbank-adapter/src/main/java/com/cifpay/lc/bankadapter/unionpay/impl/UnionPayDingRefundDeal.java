 ...
 */
@Component
public class UnionPayDingRefundDeal implements IBankDeal {

	private static Logger LOGGER = LoggerFactory.getLogger(UnionPayDingRefundDeal.class);
	...
	public String getBankCode() {
		...
		return TradeConstant.TRADE_CONFIG.TRADE_UNION_PAY
				+ TradeConstant.TRADE_CONFIG.TRADE_UNIONPAY_DINGGOU_CONSUME_REFUND;
	}
	...
	public AbsBusinessResult bankDeal(AbsTradeParam tradeParam) throws Exception {
		...
		LOGGER.info("银联订购-退货交易请求数据:\n {}", StringTool.printMap(undoRequestData));
		// 记录银联渠道交易流水信息
		...
		LOGGER.info("银联订购-退货交易请求返回数据:  {}", StringTool.printMap(responseData));
