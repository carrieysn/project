 ...
 */
@Component
public class UnionPayDingConsumeUndoDeal implements IBankDeal {

	private static Logger LOGGER = LoggerFactory.getLogger(UnionPayDingConsumeUndoDeal.class);
	...
	public String getBankCode() {
		...
		return TradeConstant.TRADE_CONFIG.TRADE_UNION_PAY
				+ TradeConstant.TRADE_CONFIG.TRADE_UNIONPAY_DINGGOU_CONSUME_UNDO;
	}
	...
	public AbsBusinessResult bankDeal(AbsTradeParam tradeParam) throws Exception {
		...
		LOGGER.info("银联订购-消费撤销请求数据！ {}", StringTool.printMap(undoRequestData));
		// 记录银联渠道交易流水信息
		...
		LOGGER.info("银联订购-消费撤销请求返回数据！ {}", StringTool.printMap(responseData));
		...
		if (!responseData.isEmpty()) {
			if (EncryptDataUtil.validate(responseData, UnionPaySettingUtil.encoding_UTF8)) {
				...
				ConsumeUndoResult consumeUndoResult = (ConsumeUndoResult) DataUtil.convertMap(ConsumeUndoResult.class,
						responseData);
				// 请求返回后 更新银联交易渠道流水信息
				...
				result.setQueryId(consumeUndoResult.getQueryId());// 原交易流水号
																	// 调用接口输入
	...
	public void updateUnionPayFlow(ConsumeUndoResult result, long flowId) {
		...
		updataFlow.setNewFlowId(result.getQueryId());// 返回的银联交易流水号
