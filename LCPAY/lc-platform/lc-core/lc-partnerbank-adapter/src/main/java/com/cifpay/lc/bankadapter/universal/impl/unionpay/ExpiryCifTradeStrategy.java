...
@Component
public class ExpiryCifTradeStrategy implements ITradeStrategy {
	private static final Logger LOGGER = LoggerFactory.getLogger(ExpiryCifTradeStrategy.class);
	@Autowired
	...
	public AbsBusinessResult process(AbsTradeParam tradeParam) throws Exception {
		LOGGER.info("ExpiryCifTradeStrategy 开始处理请求...");
		// 1. 检查参数
		new ExpiryCifValidate().validate(tradeParam);
		...
		try {
			...
			if (i == 1 || i == -1) {
				...
				if (!isAuth) {
					...
					LOGGER.info("补充相关原冻结信息-->原queryId:{}",authTm.getRtnQueryId());
			...
			LOGGER.error("####ExpiryCifTradeStrategy处理错误####：{}", e.getMessage());
			throw e;
		} finally {
	...
	public String getStrategyType() {
		return ExpiryCifParam.class.getName();
	}
